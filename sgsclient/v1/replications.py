...
class ReplicationManager(base.ManagerWithFind):
    ...
    def enable(self, replication_id):
        body = {'replication': {'replication_id': replication_id}}
        return self._create("/replications/%s/enable" % replication_id,
                            body, "replication")

    def disable(self, replication_id):
        body = {'replication': {'replication_id': replication_id}}
        return self._create("/replications/%s/disable" % replication_id,
                            body, "replication")

    def failover(self, replication_id):
        body = {'replication': {'replication_id': replication_id}}
        return self._create("/replications/%s/failover" % replication_id,
                            body, "replication")

    def reverse(self, replication_id):
        body = {'replication': {'replication_id': replication_id}}
        return self._create("/replications/%s/reverse" % replication_id,
                            body, "replication")
